...
#
    ...
    df['primary_key'] = df['tpep_dropoff_datetime'].map(str) + df['tpep_pickup_datetime'].map(str) + df['PULocationID'].map(str) + df['DOLocationID'].map(str) + df['payment_type'].map(str) + df['total_amount'].map(str)
