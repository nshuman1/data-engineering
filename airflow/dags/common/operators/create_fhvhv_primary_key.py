def fhvhv_key_gen(ti, year: str, month: str) -> None:
    """
    Function that reads a parquet file containing fhvhv data from the NYC Taxi dataset.
    ...
    for _, j in enumerate((list(path.values()))):
        ...
        if dict(j)["dest"] == "fhvhv_tripdata":
            fhvhv_path = dict(j)
            break

    filename = fhvhv_path["source"]
    dest = fhvhv_path["dest"]
    ...
    df["primary_key"] = (
        df["dropoff_datetime"].map(str)
        + df["pickup_datetime"].map(str)
        + df["PULocationID"].map(str)
        + df["DOLocationID"].map(str)
        + df["base_passenger_fare"].map(str)
        + df["driver_pay"].map(str)
    )
