...
def get_weather_data(year: str, month: str) -> None:
    ...
    weather_data = urllib.request.urlopen(f"https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/new%20york%20city/{year}-{month}-01/{year}-{month}-{last_day_of_month}?unitGroup=us&include=days&key={os.getenv('WEATHER_API')}&contentType=json")
